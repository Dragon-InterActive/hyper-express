...
const HelmetHandler = require('./middleware/HelmetHandler.js');
...
class Server extends Router {
     ...
     * @param {Boolean|Object=} options.cors Enable CORS handling. `true` for permissive defaults, or an object for custom config.
     * @param {Boolean|Object=} options.helmet Enable security headers. `true` for secure defaults, or an object for custom config.
     * @param {Boolean|Object=} options.validation Enable Zod-based request validation. `true` for defaults, or an object for custom config.
     */
    constructor(options = {}) {
        ...
        // Initialize built-in middleware handlers
        this.#cors_handler = new CorsHandler(this.#options.cors);
        this.#helmet_handler = new HelmetHandler(this.#options.helmet);
        this.#validation_handler = new ValidationHandler(this.#options.validation);
    }
    ...
    _handle_uws_request(route, uws_request, uws_response, socket) {
        ...
        // Handle CORS preflight requests (OPTIONS with Origin + Access-Control-Request-Method)
        if (this.#cors_handler.enabled && request.method === 'OPTIONS') {
            if (this.#cors_handler.preflight(request, response)) {
                return; // Preflight fully handled
            }
        }

        // Apply Helmet security headers
        if (this.#helmet_handler.enabled) {
            this.#helmet_handler.apply(request, response);
        }

        // Apply CORS headers for non-preflight requests
        if (this.#cors_handler.enabled) {
            this.#cors_handler.apply(request, response);
        }
    ...
    /**
     * Returns the CORS handler instance
     * @returns {CorsHandler}
     */
    get cors_handler() {
    ...
    /**
     * Returns the Helmet handler instance
     * @returns {HelmetHandler}
     */
    get helmet_handler() {
        return this.#helmet_handler;
    }
