...
class Route {
    ...
    constructor({ app, method, pattern, options, handler }) {
        ...
        // Store validation schemas if provided
    ...
    compile() {
        ...
        // Inject validation middleware if schemas are defined and validation is enabled
        if (this.validate && this.app.validation_handler && this.app.validation_handler.enabled) {
            const validation_handler = this.app.validation_handler;
            const schemas = this.validate;
            middlewares.push({
                id: this.id + offset + 1,
                pattern,
                handler: async (request, response, next) => {
                    // Parse body data if a body schema exists and request may have a body
                    if (schemas.body) {
                        try {
                            request._body_json = await request.json();
                        } catch (error) {
                    ...
                    // Run validation against the schemas
                    const result = validation_handler.validate(request, schemas);
