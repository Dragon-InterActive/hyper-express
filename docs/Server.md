...
* `cors` [`Boolean` | `Object`]: Enables built-in CORS handling. `true` for permissive defaults, or an object for custom configuration.
  * **Default:** `false`
  * See [`> [Cors]`](./Cors.md) for full documentation and all configuration options.
* `helmet` [`Boolean` | `Object`]: Enables built-in security headers. `true` for secure defaults, or an object for custom configuration.
  * **Default:** `false`
  * See [`> [Helmet]`](./Helmet.md) for full documentation and all configuration options.
* `validation` [`Boolean` | `Object`]: Enables built-in Zod-based request validation. `true` for defaults, or an object for custom configuration.
  * **Default:** `false`
  * **Note:** Requires the `zod` package to be installed: `npm install zod`
  * See [`> [Validation]`](./Validation.md) for full documentation and all configuration options.
...
| `cors_handler` | `CorsHandler` | CORS handler instance. |
| `helmet_handler` | `HelmetHandler` | Helmet handler instance. |
